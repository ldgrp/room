...
function addLight(
    ...
    const params = {
        ...
        distance:
            light instanceof THREE.PointLight ? light.distance : undefined!,
        angle: light instanceof THREE.SpotLight ? light.angle : undefined!,
        penumbra:
            light instanceof THREE.SpotLight ? light.penumbra : undefined!,
        width: light instanceof THREE.RectAreaLight ? light.width : undefined!,
        height:
            light instanceof THREE.RectAreaLight ? light.height : undefined!,
        position: light.position,
        lookAt: { x: 0, y: 0, z: 0 },
    };

    folder
        .addInput(params, 'intensity', { min: 0, max: 10 })
        .on('change', (ev) => {
            lightManager.lights[lightItem].intensity = ev.value;
        });

    folder.addInput(params, 'color').on('change', (ev) => {
        lightManager.lights[lightItem].color.set(ev.value as HexColorString);
    });

    if (light instanceof THREE.PointLight) {
        folder
            .addInput(params, 'distance', { min: 0, max: 10 })
            .on('change', (ev) => {
    ...
    if (light instanceof THREE.SpotLight && DEBUG) {
        folder
            .addInput(params, 'angle', { min: 0, max: Math.PI / 2 })
            .on('change', (ev) => {
                light.angle = ev.value;
            });

        folder
            .addInput(params, 'penumbra', { min: 0, max: 1 })
            .on('change', (ev) => {
    ...
    if (light instanceof THREE.RectAreaLight && DEBUG) {
        folder
            .addInput(params, 'width', { min: 0, max: 10 })
            .on('change', (ev) => {
                light.width = ev.value;
            });

        folder
            .addInput(params, 'height', { min: 0, max: 10 })
            .on('change', (ev) => {
    ...
    if (DEBUG) {
        folder
            .addInput(params.position, 'x', { min: -20, max: 20 })
            .on('change', (ev) => {
                light.position.x = ev.value;
            });

        folder
            .addInput(params.position, 'y', { min: -20, max: 20 })
            .on('change', (ev) => {
                light.position.y = ev.value;
            });

        folder
            .addInput(params.position, 'z', { min: -20, max: 20 })
            .on('change', (ev) => {
                light.position.z = ev.value;
            });

        folder
            .addInput(params.lookAt, 'x', { min: -20, max: 20 })
            .on('change', (ev) => {
                light.lookAt(ev.value, params.lookAt.y, params.lookAt.z);
            });

        folder
            .addInput(params.lookAt, 'y', { min: -20, max: 20 })
            .on('change', (ev) => {
                light.lookAt(params.lookAt.x, ev.value, params.lookAt.z);
            });

        folder
            .addInput(params.lookAt, 'z', { min: -20, max: 20 })
            .on('change', (ev) => {
